 ...
 *
 * Reference article: http://www.academia.edu/1336655/Reviews_Classification_Using_SentiWordNet_Lexicon
 */
...
object SentiWordList {
  ...
  private def processWordListFromSentiWordNet():HashMap[String, WordScore] = {
    ...
    scala.io.Source.fromInputStream(getClass.getResourceAsStream(sentWordNetPath)).getLines().foreach(auxLine => {
      // discard comments
          ...
          words.foreach(wordSense => {
            ...
            wordList.get(word) match {
              case Some(score) => {
                ...
                tmpScore = new WordScore(score.positive+postitiveScore, score.negative+negativeScore,score.count+1)
              }
              case _ =>
            }
